/*
 * Данный пример демонстрирует использование libevent и getaddrinfo в
 * серверных  программах.  Программа обслуживает telnet соединения на
 * указанных  портах/адресах  и  передает  весь  ввод сессии в другие
 * сессии.  Если   сессия  неактивна  в  течение  60  секунд,  сессия
 * завершается по таймауту.
 *
 * Запуск:
 *	eventsrv [-46d] [[адрес][:порт] ...]
 *
 *	адрес	- адрес или имя хоста, к которому выполнять  привязку
 *		  (если  не  указан  выполняется  привязка  ко   всем
 *		  адресам)
 *	порт	- порт или название TCP сервиса, к которому выполнять
 *		  привязку (если  не  указан  выполняется  привязка к
 *		  порту 32768)
 *
